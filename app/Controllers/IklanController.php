...
class IklanController extends BaseController
{
    ...
    public function store_iklan_tetap()
    {
        ...
        $totalIklanTetap = $this->iklantetap->countAll();
        if ($totalIklanTetap >= 4) {
            session()->setFlashdata('error', 'Hanya dapat menambahkan maksimal 4 iklan tetap.');
            return redirect()->to('/admin/add_iklan_tetap');
        }
        if (!$this->validate([
